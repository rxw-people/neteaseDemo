...
import {ArrowLeftOutlined,MessageOutlined,HeartOutlined,PlusOutlined,MoreOutlined,PlayCircleOutlined} from '@ant-design/icons';
...
class RankDetails extends Component {
    ...
    render() {
        return <CSSTransition in={this.state.isListDetail} timeout={2000} classNames={"rankDetail"}>
            <div className={"rankDetails"} ref="bodyBox">
                <img className={"rankDetails-img"} src={this.state.data.coverImgUrl} alt=""/>
                <header className={"rankDetails-header"} style={{backgroundColor:this.state.backColor}}>
                    <ArrowLeftOutlined onClick={()=>this.gotoUp()}  style={{backgroundColor:this.state.backColor}} />
                    <CSSTransition
                        in={this.state.isBool}
                        timeout={3000}
                        classNames={"fade"}
                    >
                    ...
                    <CSSTransition
                        ...
                        classNames={"fade"}
                    >
                        <span style={{display:this.state.nones}}>{this.state.data.name}</span>
                    </CSSTransition>
                </header>
                <div className={"rankDetails-details"}>
                    <div className={"rankDetails-details-top"}>
                    ...
                    <div className={"rankDetails-details-bottom"}>
                        <div>
                            <MessageOutlined />
                            <p>评论</p>
                        </div>
                ...
                <ul className={"rankDetails-list"}>
                    ...
                    </div>
                    {
                        this.state.dataList.map((v,i)=>{
                            return <li key={v.id} onClick={()=>this.addRankSong(v.id,v.name,v.ar[0].name,v.al.picUrl)}>
                                <p>{i+1}</p>
                                <div>
                                    <p>{v.name}</p>
    ...
    componentDidMount(){
        ...
        this.rankListerens = Store.subscribe(()=>{})
    }
    addRankSong(id,singerName,songerName,imgUrl){
        Store.dispatch(addSong(id,this.state.id,singerName,songerName,imgUrl));
        // console.log("---------------------",Store.getState().songerSing);
    }
    bindHandleScroll=(event)=>{
        // 滚动的高度
        const scrollTop = (event.srcElement ? event.srcElement.scrollTop : false) || window.pageYOffset
        if (scrollTop >= 10) {
            this.setState({
                isBool:false,
                inlines:"none",
